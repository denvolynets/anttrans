...
var Class = (function () {
	...
	function subclass () {};
	function create () {
		var parent = null; var properties = $A(arguments)
	...
	function addMethods (source) {
		...
		for (var i = 0, length = properties.length; i < length; i++) {
			var property = properties[i]; var value = source[property]
...
Object.extend(Function.prototype, (function () {
	...
	function update (array, args) {
		var arrayLength = array.length; var length = args.length
	...
	function bind (context) {
		...
		var nop = function () {}
		var __method = this; var args = slice.call(arguments, 1)

		var bound = function () {
			var a = merge(args, arguments); var c = context
	...
	function curry () {
		...
		var __method = this; var args = slice.call(arguments, 0)
		return function () {
...
Object.extend(String.prototype, (function () {
	...
	function gsub (pattern, replacement) {
		var result = ''; var source = this; var match
...
var Template = Class.create({
	...
	evaluate: function (object) {
		...
		return this.template.gsub(this.pattern, function (match) {
			...
			var ctx = object; var expr = match[3]
...
var Enumerable = (function () {
	...
	function eachSlice (number, iterator, context) {
		var index = -number; var slices = []; var array = this.toArray()
	...
	function sortBy (iterator, context) {
		return this.map(function (value, index) {
			...
			var a = left.criteria; var b = right.criteria
	...
	function zip () {
		var iterator = Prototype.K; var args = $A(arguments)
...
function $A (iterable) {
	...
	var length = iterable.length || 0; var results = new Array(length)
...
(function () {
	...
	function indexOf (item, i) {
		...
		var array = Object(this); var length = array.length >>> 0
	...
	function concat (_) {
		var array = []; var items = slice.call(arguments, 0); var item; var n = 0
		items.unshift(this)
	...
	function map (iterator) {
		...
		var results = []; var context = arguments[1]; var n = 0
	...
	function filter (iterator) {
		...
		var object = Object(this)
		var results = []; var context = arguments[1]; var value
...
var Hash = Class.create(Enumerable, (function () {
	...
	function _each (iterator, context) {
		for (var key in this._object) {
			var value = this._object[key]; var pair = [key, value]
			pair.key = key
			pair.value = value
	...
	function toQueryString () {
		return this.inject([], function (results, pair) {
			var key = encodeURIComponent(pair.key); var values = pair.value
...
(function (GLOBAL) {
   ...
   TABLE_ELEMENT_INNERHTML_BUGGY || LINK_ELEMENT_INNERHTML_BUGGY
	...
	function replace_IE (element, content) {
		...
		content = Object.toHTML(content)
		var parent = element.parentNode; var tagName = parent.tagName.toUpperCase()
		...
		if (tagName in INSERTION_TRANSLATIONS.tags) {
			...
			var iterator
			if (nextSibling) { iterator = function (node) { parent.insertBefore(node, nextSibling) } } else { iterator = function (node) { parent.appendChild(node) } }
	...
	function getContentFromAnonymousElement (tagName, html, force) {
		var t = INSERTION_TRANSLATIONS.tags[tagName]; var div = DIV
	...
	function purgeCollection_IE (elements) {
		var i = elements.length; var element; var uid
		while (i--) {
			element = elements[i]
	...
	function immediateDescendants (element) {
		var results = []; var child = $(element).firstChild
	...
	function adjacent (element) {
		element = $(element)
		var expressions = SLICE.call(arguments, 1).join(', ')
		var siblings = Element.siblings(element); var results = []
	...
	function writeAttribute (element, name, value) {
		element = $(element)
		var attributes = {}; var table = ATTRIBUTE_TRANSLATIONS.write
	...
	var classProp = 'className'; var forProp = 'for'
   ...
   'accessKey tabIndex encType maxLength readOnly longDesc frameBorder')
	...
	function setStyle (element, styles) {
		element = $(element)
		var elementStyle = element.style; var match
	...
	function retrieve (element, key, defaultValue) {
		if (!(element = $(element))) return
		var storage = getStorage(element); var value = storage.get(key)
	...
	var Methods = {}; var ByTag = Element.Methods.ByTag
...
(function () {
	...
	function getPixelValue (value, property, context) {
		...
		var isPercentage = value.include('%'); var isViewport = (context === document.viewport)

		if (/\d/.test(value) && element && element.runtimeStyle && !(isPercentage && isViewport)) {
			var style = element.style.left; var rStyle = element.runtimeStyle.left
		...
		if (element && isPercentage) {
			...
			var decimal = toDecimal(value); var whole = null
	...
	Element.Layout = Class.create(Hash, {
		...
		_begin: function () {
			...
			var position = getRawStyle(element, 'position'); var width = element.offsetWidth
			...
			var positionedWidth = element.offsetWidth; var newWidth
			if (width && (positionedWidth === width)) {
				newWidth = getContentWidth(element, context)
			} else if (position === 'absolute' || position === 'fixed') {
				newWidth = getContentWidth(element, context)
			} else {
				var parent = element.parentNode; var pLayout = $(parent).getLayout()
	...
	function viewportOffset (forElement) {
		var valueT = 0; var valueL = 0; var docBody = document.body
	...
	function makePositioned (element) {
		element = $(element)
		var position = Element.getStyle(element, 'position'); var styles = {}
	...
	function undoPositioned (element) {
		...
		var madePositioned = storage.get('prototype_made_positioned')
	...
	function makeClipping (element) {
		...
		var madeClipping = storage.get('prototype_made_clipping')
	...
	function undoClipping (element) {
		...
		var overflow = storage.get('prototype_made_clipping')
	...
	function clonePosition (element, source, options) {
		...
		source = $(source)
		element = $(element)
		var p; var delta; var layout; var styles = {}
...
Prototype.Selector = (function () {
	...
	function find (elements, expression, index) {
		index = index || 0
		var match = Prototype.Selector.match; var length = elements.length; var matchIndex = 0; var i
...
(function () {
	...
	var Sizzle = function (selector, context, results, seed) {
		...
		var m; var set; var checkSet; var extra; var ret; var cur; var pop; var i
	...
	var Expr = Sizzle.selectors = {
		...
		filters: {
			...
			text: function (elem) {
				var attr = elem.getAttribute('type'); var type = elem.type
...
var Form = {
	...
	serializeElements: function (elements, options) {
		...
		var key; var value; var submitted = false; var submit = options.submit; var accumulator; var initial
...
Form.Methods = {
	...
	getElements: function (form) {
		...
		var element; var results = []; var serializers = Form.Element.Serializers
	...
	request: function (form, options) {
		...
		var params = options.parameters; var action = form.readAttribute('action') || ''
...
Form.Element.Serializers = (function () {
	...
	function select (element, value) {
		...
		var opt; var currentValue; var single = !Object.isArray(value)
	...
	function selectMany (element) {
		var values; var length = element.length
...
(function (GLOBAL) {
   ...
   'onmouseleave' in docEl
	...
	function _element (event) {
		...
		var node = event.target; var type = event.type
	...
	function findElement (event, expression) {
		var element = _element(event); var match = Prototype.Selector.match
	...
	function unregister (element, eventName, handler) {
		...
		var i = entries.length; var entry
		while (i--) {
	...
	function stopObserving (element, eventName, handler) {
		...
		var eventNameGiven = !Object.isUndefined(eventName)
...
(function (GLOBAL) {
   ...
   John-David Dalton. */
	...
	function createResponderForCustomEvent (uid, eventName, handler) {
		return function (event) {
			var cacheEntry = Event.cache[uid]; var element = cacheEntry.element
	...
	document.getElementsByClassName = (function (instanceMethods) {
		...
		instanceMethods.getElementsByClassName = Prototype.BrowserFeatures.XPath
			? function (element, className) {
				...
				className = className.toString().strip()
				var elements = []; var classNames = (/\s/.test(className) ? $w(className) : null)
